...
export default function Slide15_BiggestFan({ stats, onAnimationComplete }: SlideProps) {
  useEffect(() => {
  ...
  if (!stats.biggestFanMoment) {
    return null;
  }

  const book = stats.biggestFanMoment;
  const disparity = stats.biggestFanDisparity || 0;
    ...
    >
      ...
      <motion.div variants={itemVariants} className="text-center max-w-xl">
        ...
        <p className="text-body-lg opacity-80">
          {disparity > 0 ? (
            <>
              That's a full <span className="font-bold">{disparity.toFixed(2)}</span> stars more
              than the average. You believe in the underdogs.
            </>
          ) : (
