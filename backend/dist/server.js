...
const scrapeBookListFromRSS = (username_1, ...args_1) => __awaiter(void 0, [username_1, ...args_1], void 0, function* (username, shelf = 'read') {
    const books = [];
    let page = 1;
    ...
    while (hasMoreItems) {
        ...
        try {
            ...
            if (items.length < 30) {
                ...
                page++;
                yield new Promise(resolve => setTimeout(resolve, 500)); // Small delay between pages
            }
        }
        catch (error) {
...
const extractYearFromRSSDate = (dateStr) => {
    if (!dateStr)
        return null;
    const match = dateStr.match(/\b(20\d{2})\b/);
    return match ? parseInt(match[1]) : null;
};
const scrapeBookGenres = (bookUrl) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    try {
        const fullBookUrl = bookUrl.startsWith('http')
            ? bookUrl
            : `https://www.goodreads.com${bookUrl}`;
        const response = yield axios_1.default.get(fullBookUrl, {
            timeout: 10000,
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
            }
        ...
        const apolloState = (_b = (_a = nextData.props) === null || _a === void 0 ? void 0 : _a.pageProps) === null || _b === void 0 ? void 0 : _b.apolloState;
        if (!apolloState)
            return [];
        // Find the book object (it has bookGenres)
        const bookKey = Object.keys(apolloState).find(key => key.startsWith('Book:') && apolloState[key].bookGenres);
        if (!bookKey)
            return [];
        const nonGenres = [
        ...
        const genres = bookGenres
            .map((g) => g.genre.name.toLowerCase())
            .filter((name) => !nonGenres.includes(name));
...
const analyzeReviewsWithSentiment = (books) => __awaiter(void 0, void 0, void 0, function* () {
...
const scrapeToReadList = (username, year) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const allToReadBooks = yield scrapeBookListFromRSS(username, 'to-read');
...
app.get('/scrape/:username', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const response = yield axios_1.default.get(goodreadsUrl);
...
app.get('/scrape/:username/books/:year', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const username = req.params.username;
        ...
        const allRSSBooks = yield scrapeBookListFromRSS(username, 'read');
        ...
        const genrePromises = yearBooks.map((book, index) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (bookId) {
                ...
                const genres = yield scrapeBookGenres(bookUrl);
                book.genres = genres;
        ...
        const toReadIds = yield scrapeToReadList(username, year);
        // Count books read in year that were also added in year
        const readAndAddedInYear = yearBooks.filter(book => book.dateAdded && book.dateAdded.includes(year)).length;
        ...
        const slowestRead = booksWithReadingTime.length > 0
            ...
            : null;
        console.log(`Reading time stats: Average ${averageReadingTime.toFixed(2)} days, Fastest: ${fastestRead === null || fastestRead === void 0 ? void 0 : fastestRead.title} (${fastestRead === null || fastestRead === void 0 ? void 0 : fastestRead.readingDays} days), Slowest: ${slowestRead === null || slowestRead === void 0 ? void 0 : slowestRead.title} (${slowestRead === null || slowestRead === void 0 ? void 0 : slowestRead.readingDays} days)`);
        // Calculate biggest hater moment (biggest negative disparity between user rating and average rating)
        const booksWithBothRatings = yearBooks.filter(book => book.userRating !== undefined && book.avgRating !== undefined);
        let biggestHaterMoment = null;
        ...
        const shortestBook = booksWithPages.length > 0
            ...
            : null;
        console.log(`Page stats: Average ${averagePages.toFixed(0)} pages, Longest: ${longestBook === null || longestBook === void 0 ? void 0 : longestBook.title} (${longestBook === null || longestBook === void 0 ? void 0 : longestBook.numPages} pages), Shortest: ${shortestBook === null || shortestBook === void 0 ? void 0 : shortestBook.title} (${shortestBook === null || shortestBook === void 0 ? void 0 : shortestBook.numPages} pages)`);
        // Analyze sentiment for existing reviews
        console.log(`Analyzing sentiment for ${yearBooks.length} books...`);
        const booksWithSentiment = yield analyzeReviewsWithSentiment(yearBooks);
        // Find the most scathing review (most negative sentiment)
        const booksWithReviews = booksWithSentiment.filter(book => book.sentiment && book.sentiment.fullReview && book.sentiment.fullReview.length > 20);
        const mostScathingReview = booksWithReviews.length > 0
            ? booksWithReviews.reduce((mostScathing, book) => book.sentiment.comparative < mostScathing.sentiment.comparative ? book : mostScathing)
            : null;
        ...
        const mostPositiveReview = booksWithReviews.length > 0
            ? booksWithReviews.reduce((mostPositive, book) => book.sentiment.comparative > mostPositive.sentiment.comparative ? book : mostPositive)
            : null;
